...
binaryField AppConfig{configRawMediaTypes} acceptMediaType proc rpTree
  ...
  where
    ...
    fstFieldName (Node ReadPlan{select=[CoercibleSelectField{csField=CoercibleField{cfName="*", cfJsonPath=[]}}]} [])  = Nothing
    fstFieldName (Node ReadPlan{select=[CoercibleSelectField{csField=CoercibleField{cfName=fld, cfJsonPath=[]}}]} [])  = Just fld
